...
L = [exp(-0.2*dU*s), 0;             % nominal delay matrix
     0, exp(-0.4*dU*s)];
S = minreal(G1*(eye(2)-L), 1e-5);   % nominal plant loop
...
Pz = Gz*Lz;                         % nominal delayed plant
Sz = c2d(S,Ts);                     % discretize nominal plant loop
Fz = c2d(F1,Ts);

% simulation to workspace
sim = sim("smithPredictorExample1Simulink.slx");
...
title('Unit step response'), hold off;
    % plot y2
subplot(2, 2, 2);
